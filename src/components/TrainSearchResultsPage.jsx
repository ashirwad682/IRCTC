...
import { ArrowLeft, Filter, ArrowUpDown, ChevronLeft, ChevronRight, RotateCw, Info, ArrowLeftRight, Edit3, ChevronDown, Calendar, Search } from 'lucide-react';
...
export default function TrainSearchResultsPage({ fromStation: initialFrom, toStation: initialTo, selectedDate: initialDate, selectedQuota: initialQuota, selectedClass: initialClass, onModifySearch, onSelectTrainClass, onLiveTrack }) {
  ...
  return (
    <div className="min-h-screen bg-[#eff4fe] text-slate-900 pb-12">
      ...
      {/* Horizontal Search Modification Sticky Header */}
      <div className="bg-white border-b border-slate-200 py-2.5 shadow-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 space-y-2">
          
          {/* Top Row: Back button & Summary Pill (Mobile Collapsed View) */}
          <div className="flex items-center justify-between gap-2">
            <button
              onClick={onModifySearch}
              className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-blue-950 font-black text-xs transition-colors flex items-center gap-1.5 cursor-pointer shrink-0 border border-slate-200"
            >
              <ArrowLeft className="w-4 h-4 text-blue-950" />
              <span>Back</span>
            </button>

            {/* Compact Mobile Summary Pill */}
            <div
              onClick={() => setIsModifyExpanded(!isModifyExpanded)}
              className="sm:hidden flex-1 flex items-center justify-between gap-2 bg-blue-50/80 border border-blue-200 px-3 py-1.5 rounded-xl text-xs cursor-pointer hover:bg-blue-100/60 transition-all"
            >
              <div className="truncate font-black text-blue-950 text-[11px]">
                <span>{fromObj.code || fromStation}</span>
                <span className="text-blue-600 mx-1">➔</span>
                <span>{toObj.code || toStation}</span>
                <span className="text-slate-400 font-bold ml-1.5">| {selectedDate}</span>
              </div>
              <div className="flex items-center gap-1 text-[10px] font-black text-blue-800 shrink-0 bg-white px-2 py-0.5 rounded-md border border-blue-200 shadow-2xs">
                <Edit3 className="w-3 h-3 text-blue-700" />
                <span>{isModifyExpanded ? 'Close' : 'Modify'}</span>
              </div>
            </div>
          </div>

          {/* Full Inputs Pill (Expanded on Mobile or always visible on Desktop) */}
          <div className={`${isModifyExpanded ? 'block' : 'hidden'} sm:block animate-in fade-in zoom-in-95 duration-150`}>
            <div className="bg-slate-50 p-2 sm:p-2.5 rounded-2xl border border-slate-200 space-y-2.5 shadow-2xs">
              
              {/* Inputs Container */}
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                
                {/* FROM Autocomplete */}
                <div className="min-w-0 sm:min-w-[200px] flex-1 relative z-30">
                  <StationAutocomplete
                    label="From Station"
                ...
                <button
                  ...
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white hover:bg-blue-50 border border-slate-300 hover:border-blue-500 text-blue-900 flex items-center justify-center shrink-0 self-center transition-all cursor-pointer shadow-xs hover:scale-105 active:scale-95 my-0.5 sm:my-0"
                  title="Swap Origin & Destination Stations"
                >
                  <ArrowLeftRight className="w-4 h-4 text-[#000066]" />
                </button>
                ...
                <div
                  ...
                  className="bg-white p-2.5 rounded-2xl border border-slate-200 text-xs font-bold min-w-0 sm:min-w-[130px] cursor-pointer hover:border-blue-600 transition-all flex items-center justify-between gap-2 shadow-2xs"
                >
                  <div>
                    <span className="text-[9px] text-slate-400 font-black uppercase block">Date</span>
                    <span className="font-extrabold text-blue-950 text-xs block">
                      {selectedDate || 'Select Date'}
                    </span>
                  </div>
                  <span className="text-[10px] text-blue-700 bg-blue-50 px-2 py-1 rounded-lg font-black shrink-0 border border-blue-200">📅 Change</span>
                </div>

                {/* QUOTA */}
                <div className="bg-white p-2 rounded-2xl border border-slate-200 text-xs font-bold min-w-0 sm:min-w-[135px] shadow-2xs">
                  <span className="text-[9px] text-slate-400 font-black uppercase flex items-center justify-between">
                    Quota
                  ...
                  <select
                    ...
                    className="w-full bg-transparent font-black text-orange-600 text-xs focus:outline-none cursor-pointer pt-0.5"
                  >
                    {isTatkalWindow ? (
                      <>
                ...
                <button
                  ...
                  className="px-6 py-3 rounded-2xl bg-[#000066] hover:bg-blue-900 text-white font-black text-xs shadow-md transition-all active:scale-95 cursor-pointer w-full sm:w-auto shrink-0 flex items-center justify-center gap-1.5"
                >
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 pt-3 pb-3">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 bg-white p-2 rounded-2xl border border-slate-200 shadow-2xs mb-3">
          
          <div className="flex items-center gap-1.5 flex-1 min-w-0">
            <button
              onClick={() => {
                const el = document.getElementById('date-carousel-strip');
                if (el) el.scrollBy({ left: -200, behavior: 'smooth' });
              }}
              className="p-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-600 shrink-0 cursor-pointer"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>

            <div id="date-carousel-strip" className="flex items-center gap-2 overflow-x-auto py-0.5 scrollbar-none flex-1 min-w-0 scroll-smooth">
              {dateTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs text-center transition-all shrink-0 min-w-[115px] cursor-pointer ${
                      isSelected
                        ? 'bg-[#e8eaf6] text-[#283593] font-black border border-[#c5cae9] shadow-2xs'
                        : 'bg-white text-slate-700 font-bold hover:bg-slate-50 border border-slate-200'
                    }`}
                  >
                    <span className="block text-[11px] font-black whitespace-nowrap">{tab.label}</span>
                    <span className="text-[10px] text-slate-500 font-bold whitespace-nowrap">{tab.count} Train{tab.count !== 1 ? 's' : ''}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t sm:border-t-0 sm:border-l border-slate-200 pt-2 sm:pt-0 sm:pl-2 shrink-0">
            <button
            ...
            <button className="px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-800 font-bold text-xs flex items-center gap-1.5 border border-slate-300 cursor-pointer">
